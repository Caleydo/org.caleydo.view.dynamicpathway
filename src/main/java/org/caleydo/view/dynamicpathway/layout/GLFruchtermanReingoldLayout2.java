...
public class GLFruchtermanReingoldLayout2 implements IGLLayout2 {
	...
	private Map<IFRLayoutNode, Point2D> dispMap;
	...
	public GLFruchtermanReingoldLayout2(int maxIterations, double temperature, double cooldown, double repulsionMultiplier, double attractionMultiplier) {
		...
		/**
		 * Temperature & cooldown are set to -1.0 by default, 
		 * so if these are not -1.0, the user set them by the LayoutBuilder
		 */
		if(temperature >= 0.0) {
	...
	public boolean doLayout(List<? extends IGLLayoutElement> children, float w, float h,
		...
		// Map<DefaultEdge, Pair<PathwayVertexRep, PathwayVertexRep>> edges =
		// graph.getCombinedEdges();
	 ...
	 * 
	 * @param children
	 *            renderable children objects of the container, which contain the actual Nodes
	 * @param currentNode
	...
	private void calcNewVertexPositions(NodeElement currentNode, IGLLayoutElement child) {
		...
		// currentNode.setCoords((short)newXPos, (short)newYPos,
		// currentNode.getVertex().getWidth(),
		// currentNode.getVertex().getHeight());
